...
create_symbolic_link() {
	...
	if [ -d "${S}" ]; then
		if [ -L "${T}" ]; then
			...
			rm -rf "${T}"
			if [ $? == 0 ]; then echo `date '+%Y-%m-%d %H:%M:%S'` "[L302:$APP] Deleted target with same name as symbolic link \"${T}\"" >> ~/$LOG; fi
		fi
		# Creates the symbolic link
		ln -s "${S}" "${T}"
		if [ $? == 0 ]; then echo `date '+%Y-%m-%d %H:%M:%S'` "[L303:$APP] Created symbolic \"${T}\" (Points to \"${S}\")" >> ~/$LOG; fi
		#echo "Symbolic link '${T}' created."
	else
		#echo "ERROR: S '${S}' does not exist."
		echo `date '+%Y-%m-%d %H:%M:%S'` "[L304:$APP] ERROR: Source \"${S}\" does not exist" >> ~/$LOG
	fi
}
